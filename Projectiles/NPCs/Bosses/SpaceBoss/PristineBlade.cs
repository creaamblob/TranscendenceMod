...
namespace TranscendenceMod.Projectiles.NPCs.Bosses.SpaceBoss
{
    public class PristineBlade : ModProjectile
    {
        ...
        {
            if (targetHitbox.Distance(Projectile.Center) < 15 && Projectile.ai[2] > 5)
                return true;
            else return false;
        }
        ...
        {
            float fade = Projectile.ai[2] < 45 ? Projectile.ai[2] / 45f : 1f;

            if (Projectile.ai[2] > 90)
            {
                TranscendenceUtils.DrawTrailProj(Projectile, Color.Gold * 0.5f, Projectile.scale * 3f, "TranscendenceMod/Miscannellous/Assets/Trail2", false, true, 2f, Vector2.Zero);
