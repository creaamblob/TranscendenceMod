...
{
    public class PoCLaser : ModProjectile
    {
        public int Timer;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            eff.Parameters["extraCol"].SetValue(new Vector3(2f, 0.75f, 0f));
            ...
            for (int i = 4; i < ((Projectile.oldPos.Length - 2)); i++)
            {
                ...
                Main.EntitySpriteDraw(sprite4, pos, null, Color.White * a, Projectile.oldRot[i] + MathHelper.PiOver2, sprite4.Size() * 0.5f, Projectile.scale * 2f, SpriteEffects.None);
            }
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft > 120 && Projectile.ai[1] < 1f)
                Projectile.ai[1] += 0.0125f;
            else if (Projectile.timeLeft < 30f && Projectile.ai[1] > 0f)

                Projectile.ai[1] -= 1f / 30f;
            if (++Timer > 3)
            {
                Projectile.velocity = Projectile.velocity.RotatedBy(MathHelper.Pi / (60 * Projectile.ai[2]));
                Timer = 0;
            }
        }
    }
