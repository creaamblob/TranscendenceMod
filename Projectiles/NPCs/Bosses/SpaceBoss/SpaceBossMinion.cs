...
namespace TranscendenceMod.Projectiles.NPCs.Bosses.SpaceBoss
{
    public class SpaceBossMinion : ModProjectile
    {
        ...
        {
            int proj = ModContent.ProjectileType<StellarFireball>();

            if (Main.LocalPlayer.GetModPlayer<TranscendencePlayer>().RoHTeleports > 4)
            {
                Enraged = true;
            }
            if (Main.npc[(int)Projectile.ai[1]].ModNPC is CelestialSeraph boss)
            {
                ...
                if (++Projectile.localAI[2] > 60)
                {
                    for (int i = 0; i < Main.maxPlayers; i++)
                    {
                        ...
                        if (player != null && player.active && player.Distance(Projectile.Center) < 1200 && (Main.expertMode || Main.masterMode) && !Charged && (boss.Phase > 1 || boss.Timer_AI > 320) && Projectile.localAI[2] < 65)
                        {
                            pos = player.Center;
                ...
                else
                {
                    if (++CD > Projectile.ai[2])
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            if (boss.Phase == 2) pa.velocity.X = 2;
                            pa.GetGlobalProjectile<TranscendenceProjectiles>().StellarDirection = 1;
                            pa.extraUpdates = 2;
                            ...
                            if (boss.Phase == 2) pb.velocity.X = -2;
                            pb.GetGlobalProjectile<TranscendenceProjectiles>().StellarDirection = -1;
                            pb.extraUpdates = 2;
                        }
                        CD = 0;
                    }
