...
{
    public class MeteorCompass : ModItem
    {
        public int Amount = 0;
        public Vector2 Pos;
        ...
        public override void HoldItem(Player player)
        {
            if (Pos != Vector2.Zero && player.ItemTimeIsZero)
                Dust.QuickDustLine(player.Center + new Vector2(22 * player.direction, 4), player.Center + new Vector2(22 * player.direction, 4) + Vector2.One.RotatedBy(player.DirectionTo(Pos).ToRotation() - MathHelper.PiOver4) * 5, 50, Color.Orange);
            ...
            if (player.direction == 1)
            {
                for (int i = x; i < xDest; i++)
                {
                    for (int j = 55; j < (Main.maxTilesY - 55); j++)
                    {
                        ...
                        {
                            Amount++;
                            if (Amount >= 75) Pos = new Vector2(i, j) * 16;
                        }
                    }
                }
            }
