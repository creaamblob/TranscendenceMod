...
namespace TranscendenceMod.Items.Weapons
{
    ...
    {
        int proj = ModContent.ProjectileType<ElementalBowProj>();
    ...
    public class ElementalBowProj : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (TimeSinceSpawn > 5)
            {
                if (Main.MouseWorld.Distance(pos) > 60 && player != null && ChargeTimeForShot > 0)
                {
                    ...
                    Color col = Projectile.ai[1] > (ChargeTimeForShot - 5) ? Color.Magenta : lightColor;

                    Utils.DrawLine(Main.spriteBatch, pos2, pos3, col, col, 2);
                    Utils.DrawLine(Main.spriteBatch, pos4, pos5, col, col, 2);

                    Main.EntitySpriteDraw(sprite, Projectile.Center - Main.screenPosition, null, lightColor, Projectile.rotation, sprite.Size() * 0.5f, 1,
    Main.MouseWorld.X < Projectile.Center.X ? SpriteEffects.FlipVertically : SpriteEffects.None);
        ...
        public override void AI()
        {
            ...
            if (player.HeldItem.type != ModContent.ItemType<ElementalBow>() || player.GetModPlayer<TranscendencePlayer>().CannotUseItems || player.dead)
                Projectile.Kill();
