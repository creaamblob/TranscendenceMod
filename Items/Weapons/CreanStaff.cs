    ...
    public class CreanStaffProj : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (TimeSinceSpawn > 5)
            {
                ...
                {
                    float rot2 = dir == -1 ? MathHelper.PiOver2 : 0f;
                    Main.EntitySpriteDraw(sprite, Projectile.Center - Main.screenPosition, null, Color.White, Projectile.rotation + player.fullRotation + MathHelper.PiOver2 + rot2, sprite.Size() * 0.5f, 1f,
                        SpriteEffects.None);
                }
            }
            return false;
        ...
        public override void AI()
        {
            ...
            float quarter = (dir == 1 ? MathHelper.PiOver4 : -MathHelper.PiOver4);
            float rotation = (player.MountedCenter - Main.MouseWorld).ToRotation() + MathHelper.PiOver2 + quarter - player.fullRotation;
            ...
            if (Main.MouseWorld.Distance(player.Center) > 75)
            {
                player.SetCompositeArmFront(true, Player.CompositeArmStretchAmount.Full, rotation - quarter);
                ...
                if (player.controlUseItem && Projectile.ai[2] == 0)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), vec, vel * speed, ModContent.ProjectileType<CreanLaser>(), player.HeldItem.damage / 2, Projectile.knockBack, player.whoAmI, 0f, 1f, -1f);
