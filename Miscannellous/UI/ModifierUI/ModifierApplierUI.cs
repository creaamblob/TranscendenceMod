...
namespace TranscendenceMod.Miscannellous.UI.ModifierUI
{
    public class ModifierApplierUIState : UIState
    {
        ...
        public float ValueCoins()
        {
            ...
            if (IsACraftingRecipe(npc, moddable.Item2, mod.Item2, moddable.Item2.type, mod.Item2.type))
            {
        ...
        private void Panel_OnUpdate(UIElement affectedElement)
        {
            ...
            else
            {
                ...
                if (moddable.Item2 != null && !moddable.Item2.IsAir && mod.Item2 != null && !mod.Item2.IsAir && mod.Item2.ModItem is BaseModifier modifier)
                {
                    ...
                    if (IsACraftingRecipe(npc, moddable.Item2, mod.Item2, moddable.Item2.type, mod.Item2.type) && modifier.CraftingResultItem > 0)
                    {
                ...
                if (moddable.Item2 != null && !moddable.Item2.IsAir && mod.Item2 != null && !mod.Item2.IsAir && mod.Item2.ModItem is ModifierContainer cont)
                {
                    ...
                    if (it != null && !it.IsAir && it.ModItem is BaseModifier mod2)
                    {
                        ...
                        if (IsACraftingRecipe(npc, moddable.Item2, mod.Item2, moddable.Item2.type, mod.Item2.type) && mod2.CraftingResultItem > 0)
                        {
        ...
        private void Button2_OnLeftClick(UIMouseEvent evt, UIElement listeningElement)
        {
            ...
            if (CanApply() && mod.Item2.ModItem is BaseModifier bMod)
            {
                ...
                TryCrafting(npc, moddable.Item2, mod.Item2);
        ...
        public void TryCrafting(NPC npc, Item affectedWeapon, Item upgradeItem)
        {
            //Resulting items are set in the upgrade item's .cs files
            AddModifierUpgrades(npc, affectedWeapon, upgradeItem, ItemID.Muramasa, ModContent.ItemType<MuramasaUpgrade>());
            AddModifierUpgrades(npc, affectedWeapon, upgradeItem, ModContent.ItemType<BaseballBat>(), ModContent.ItemType<HardmetalSpike>());
            AddModifierUpgrades(npc, affectedWeapon, upgradeItem, ItemID.ClockworkAssaultRifle, ModContent.ItemType<ClockworkCog>());
            AddModifierUpgrades(npc, affectedWeapon, upgradeItem, ItemID.EyeoftheGolem, ModContent.ItemType<SnowOfInfinity>());
            AddModifierUpgrades(npc, affectedWeapon, upgradeItem, ItemID.RocketLauncher, ModContent.ItemType<SpaceScrap>());
        }
        public bool IsACraftingRecipe(NPC npc, Item affectedWeapon, Item upgradeItem, int affectedWeaponType, int upgradeItemType)
        {
            return (affectedWeapon.type == affectedWeaponType && upgradeItem.type == upgradeItemType);
        }
        public void AddModifierUpgrades(NPC npc, Item affectedWeapon, Item upgradeItem, int affectedWeaponType, int upgradeItemType)
        {
            if (IsACraftingRecipe(npc, affectedWeapon, upgradeItem, affectedWeaponType, upgradeItemType) && upgradeItem.ModItem is BaseModifier modifier)
            {
