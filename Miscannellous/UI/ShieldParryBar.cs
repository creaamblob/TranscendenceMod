...
namespace TranscendenceMod.Miscannellous.UI
{
    public class ShieldParryBar : UIElement
    {
        ...
        protected override void DrawSelf(SpriteBatch spriteBatch)
        {
            ...
            if (player != null && player.active && player.GetModPlayer<TranscendencePlayer>().Parry > 0 && (timer > 0 || onCD || player.GetModPlayer<TranscendencePlayer>().BrokenShield))
            {
                int x = (int)pos.X;
                ...
                if (player.GetModPlayer<TranscendencePlayer>().ParryTimerCD > 1)
                {
                    for (int i = 0; i < 8; i++)
                    {
                ...
                if (Hover)
                {
                    Main.hoverItemName = Language.GetTextValue("Mods.TranscendenceMod.Messages.Parry");
    ...
    public class ParryState : UIState
    {
...
namespace TranscendenceMod.Miscannellous.UI
{
    ...
    {
        internal ParryState parryBar;
        private UserInterface parryBar2;

        public override void Load()
        {
            parryBar = new ParryState();
            parryBar.Activate();
        ...
        public override void ModifyInterfaceLayers(List<GameInterfaceLayer> layers)
        {
            ...
            if (mouseTextIndex != -1)
            {
                layers.Insert(mouseTextIndex, new LegacyGameInterfaceLayer("TranscendenceMod: Parry Leniency Helper", delegate
                {
