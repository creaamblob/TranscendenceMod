...
namespace TranscendenceMod
{
    public class TranscendenceWorld : ModSystem
    {
        ...
        public override void ModifySunLightColor(ref Color tileColor, ref Color backgroundColor)
        {
            ...
            if (Main.eclipse)
            {
                tileColor = Color.DarkOrange * 0.4f;
                backgroundColor = new Color(35, 25, 15);
            }
        ...
        public override void PostAddRecipes()
        {
            for (int r = 0; r < Main.recipe.Length; r++)
            {
                ...
                if (recipe.createItem.type == ItemID.DeerThing)
                    recipe.DisableRecipe();

                if (recipe.createItem.type == ItemID.LunarBar)
                    recipe.DisableRecipe();
            }

            Recipe deerThing = Recipe.Create(ItemID.DeerThing);
            deerThing.AddIngredient(ItemID.SnowBlock, 12);
            deerThing.AddIngredient(ItemID.Leather, 4);
            deerThing.AddIngredient(ItemID.Lens);
            deerThing.AddTile(TileID.DemonAltar);
            deerThing.Register();

            Recipe luminiteBar = Recipe.Create(ItemID.LunarBar, 2);
            luminiteBar.AddIngredient(ItemID.LunarOre, 6);
            luminiteBar.AddIngredient(ItemID.FallenStar);
            luminiteBar.AddTile(TileID.AdamantiteForge);
            luminiteBar.Register();
        }
        ...
        private void BossChecklistCrossMod()
        {
            ...
            var SpacePortrait = (SpriteBatch spriteBatch, Rectangle rec, Color color) =>
            {
                ...
                Vector2 pos = new Vector2(rec.X + (rec.Width * 0.7f) - (sprite.Width / 2), rec.Y + (rec.Height / 2) - (sprite.Height / 2));
