...
namespace TranscendenceMod.Miscannellous.GlobalStuff
{
    public class TranscendenceProjectiles : GlobalProjectile
    {
        ...
        public override void AI(Projectile projectile)
        {
            ...
            /*if (projectile.type == ModContent.ProjectileType<SpiritHostile>() && projectile.Distance(TranscendenceWorld.WorldCenter) > (3000 * 1.35f))
                projectile.Kill();*/
        ...
        public override bool PreDraw(Projectile projectile, ref Color lightColor)
        {
            ...
            if (projectile.type == ProjectileID.PhantasmalDeathray)
            {
                ...
                Vector2 pos = owner.Center + Vector2.One.RotatedBy(projectile.velocity.ToRotation() - 0.8f) * 50f;
                Vector2 pos2 = projectile.Center + Vector2.One.RotatedBy(projectile.velocity.ToRotation() - 0.8f) * 1850f;
                ...
                eff.Parameters["extraCol"].SetValue(new Vector3(0f, 0.85f, 0.5f));
                ...
                Color col = Color.Lerp(Color.Blue, Color.DeepSkyBlue * 0.75f, sin);

                int am = projectile.scale < 1f ? 8 : 1;
                float am2 = projectile.scale < 1f ? 1.25f : 1f;

                for (int i = 0; i < am; i++)
                {
                    sb.Draw(sprite, new Rectangle(
                        (int)(pos.X - Main.screenPosition.X), (int)(pos.Y - Main.screenPosition.Y), (int)(185 * am2 * projectile.scale), (int)(pos.Distance(pos2) * 2f)), null,
                        col * (1.25f + (sin * 0.125f)) * projectile.scale, pos.DirectionTo(pos2).ToRotation() + MathHelper.PiOver2, sprite.Size() * 0.5f, SpriteEffects.None, 0);
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    eff.Parameters["uImageSize1"].SetValue(shaderImage.Size() / 2.25f);
