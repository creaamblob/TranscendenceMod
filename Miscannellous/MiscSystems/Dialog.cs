        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            for (int i = 0; i < Dialog.Count; i++)
            {
                if (font2 != null && Dialog[i].UnfinishedSentence != null && Dialog[i].UnfinishedSentence != "" && Dialog[i].UnfinishedSentence.Length > 0)
                {
                    ...
                    float a = (Dialog[i].MaxTimeLeft - Dialog[i].TimeLeft) / (float)Dialog[i].MaxTimeLeft;

                    ChatManager.DrawColorCodedStringWithShadow(spriteBatch, font2, Dialog[i].UnfinishedSentence, Dialog[i].Center - new Vector2(x, 0) - Main.screenPosition, Dialog[i].Color * a, 0f, Vector2.Zero, Vector2.One);
