...
namespace TranscendenceMod
{
    ...
    {
        public static int MaxDialog => 100;
            ...
            for (int i = 0; i < Dialog.Count; i++)
            {
                if (font2 != null && Dialog[i].UnfinishedSentence != null && Dialog[i].UnfinishedSentence != "" && Dialog[i].UnfinishedSentence.Length > 0)
                {
                    ...
                    float a =
                        ...
                        Dialog[i].TimeLeft < (Dialog[i].MaxTimeLeft / 2f) ? 1f :
                        // The Fadeout Effect
                        ((Dialog[i].MaxTimeLeft - Dialog[i].TimeLeft) / (float)Dialog[i].MaxTimeLeft) * 2f;

                    Vector2 pos = Dialog[i].Center - new Vector2(x, 0) - Main.screenPosition;
                    if (Dialog[i].ScreenLock)
                    {
                        pos = Dialog[i].Center - new Vector2(x, 0);

                        Color def = new Color(63, 65, 151, 255) * 0.785f;

                        if (a > 0.1f)
                            Utils.DrawInvBG(spriteBatch, new Rectangle((int)pos.X - 10, (int)pos.Y - 5, (int)x * 2 + 20, 40), def * a);
                        ChatManager.DrawColorCodedStringWithShadow(spriteBatch, font2, Dialog[i].UnfinishedSentence, pos, Dialog[i].Color * a, 0f, Vector2.Zero, Vector2.One);
                    }
                    else
                        ChatManager.DrawColorCodedStringWithShadow(spriteBatch, font2, Dialog[i].UnfinishedSentence, pos, Dialog[i].Color * a, 0f, Vector2.Zero, Vector2.One);
    ...
    public class Dialog
    {
        ...
        public void Update()
        {
            ...
            if (SentenceProgress >= CurrentSentence.Length)
            {
            ...
            else
            {
                if (++Timer % 5 == 0)
                {
                    UnfinishedSentence += CurrentSentence[SentenceProgress];
                    SentenceProgress++;
                }
            }
        }
