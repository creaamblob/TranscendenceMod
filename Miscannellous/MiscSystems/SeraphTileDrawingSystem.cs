...
namespace TranscendenceMod.Miscanellous.MiscSystems
{
    public class SeraphTileDrawingSystem : ModSystem
    {
        ...
        public override void Load()
        {
            ...
            //Misc Updates
            On_WorldGen.UpdateWorld += On_WorldGen_UpdateWorld;
        ...
        private void On_Framing_SelfFrame8Way(On_Framing.orig_SelfFrame8Way orig, int i, int j, Tile centerTile, bool resetFrame)
        {
            if (!PhaseThrough)
                orig(i, j, centerTile, resetFrame);
        }

        private void On_WorldGen_Reframe(On_WorldGen.orig_Reframe orig, int x, int y, bool resetFrame)
        {
            if (!PhaseThrough)
                orig(x, y, resetFrame);
        }
        ...
        private void On_WorldGen_UpdateWorld(On_WorldGen.orig_UpdateWorld orig)
        {
            orig();
        }
        ...
        private Tile On_Framing_GetTileSafely_int_int(On_Framing.orig_GetTileSafely_int_int orig, int i, int j)
        {
            if (PhaseThrough)
                return default;
