...
namespace TranscendenceMod
{
    public class TranscendencePlayer : ModPlayer
    {
        ...
        public int VNecklaceLifeLoss;
        public int PlayerCrimsonNecklaceTimer;
        ...
        /*Environment*/
        public bool CanSpawnLesserSeraph;
        ...
        public override void OnHurt(Player.HurtInfo info)
        {
            ...
            if (Vampire)
            {
                VampireBlood = (int)(VampireBlood * 0.9f);
            }
        }
        public override bool PreKill(double damage, int hitDirection, bool pvp, ref bool playSound, ref bool genGore, ref PlayerDeathReason damageSource)
        {
            ...
            {
                if (MeltingBlood) damageSource = PlayerDeathReason.ByCustomReason(NetworkText.FromKey("Mods.TranscendenceMod.Messages.Death.MagmaBlood", Player.name));
        ...
        public override void PreUpdateMovement()
        {
            ...
            if (InsideShell == 0 && !InsideGolem && FishTrans == 0 && !Possessing)
            {
        ...
        public override void PostHurt(HurtInfo info)
        {
            ...
            if (Vampire)
            {
                VNecklaceLifeLoss = info.Damage;
                Player.Heal(VNecklaceLifeLoss);
                if (!BloodDying) Player.AddBuff(ModContent.BuffType<BloodLoss>(), VNecklaceLifeLoss);
                else PlayerCrimsonNecklaceTimer += VNecklaceLifeLoss;
            }
        ...
        public override void PreUpdate()
        {
            ...
            {
                if (DeathBombCD > 0) DeathBombCD--;
                else if (DeathBombCD == 1) SoundEngine.PlaySound(SoundID.MaxMana, Player.Center);
            }
            /*if (Player.ownedProjectileCounts[ModContent.ProjectileType<EolTransPrisma>()] == 0 && EverglowingCrownEquipped)
            {
                Projectile.NewProjectile(Player.GetSource_FromAI(), Player.MountedCenter, Vector2.Zero, ModContent.ProjectileType<EolTransPrisma>(), 130, 7f, Player.whoAmI);
            }*/
        ...
        public void SetStats()
        {
            ...
            if (HasJellyBuff && Player.HasBuff(ModContent.BuffType<JellyBuff>()))
            {
                ...
                float sightMult = buffTime > 300 ? 0.5f : MathHelper.Lerp(0f, 0.5f, buffTime / 300f);
            ...
            else
            {
                ...
                Filters.Scene["TranscendenceMod:ScreenVignette"].GetShader().UseIntensity(1f * vignetteStrengthMult * NullFade);
            ...
            if (Vampire)
            {
                ...
                if (Player.statDefense < 100)
                    Player.statLifeMax2 += 300 - (Player.statDefense * 3);
            }
            ...
            if (RoHTeleports > 0)
            {
                if (++RoHTimer > 900)
                {
                    ...
                    Main.NewText($"-1 Teleport, Current: {RoHTeleports}", Color.Gold);
                    RoHTimer = 0;
                }
            }
        ...
        {
            if (GiantSlayer > 0 && target.boss)
                modifiers.FinalDamage *= (1f + (GiantSlayer / 10f));
        ...
        public override void ProcessTriggers(TriggersSet triggersSet)
        {
            ...
            if (TranscendenceWorld.RetLensKeybind.JustPressed) NucleusLensKeybind = true;
        ...
        public override void CatchFish(FishingAttempt attempt, ref int itemDrop, ref int npcSpawn, ref AdvancedPopupRequest sonar, ref Vector2 sonarPosition)
        {
            ...
            if (ZoneLandSite || ZoneStar)
            {
                if (attempt.common)
                {
                    ...
                    {
                       ModContent.ItemType<ExtraTerrestrialString>(),
                       ModContent.ItemType<SpaceRockItem>(),
                ...
                if (attempt.rare)
                {
                    ...
                    itemDrop = rareItems[Main.rand.Next(rareItems.Count)];
                }
                /*if (attempt.legendary)
                {
                    List<int> legendaryItems = new List<int>()
                    {
                        ...
                        ModContent.ItemType<QuantumSlicer>(),
                        ModContent.ItemType<Spaceyo>()
                    };

                    itemDrop = legendaryItems[Main.rand.Next(legendaryItems.Count)];
                }*/
            ...
            if (attempt.legendary && !attempt.inHoney && !attempt.inLava && Main.hardMode && Main.bloodMoon)
            {
                itemDrop = ModContent.ItemType<Dreadshot>();
            }
        }
        public override void OnHitByNPC(NPC npc, HurtInfo hurtInfo)
        {
            ...
            if (npc.ModNPC is FrostSerpent_Head worm && InsideShell > 0)
            {
                worm.SizeMult = BeetleShield ? 1.0125f : 1.25f;
                worm.SizeResetCD = 90;

                if (TurtleShield)
                    npc.SimpleStrikeNPC(41054, 1, true, 0, DamageClass.Default, true, Player.luck);

                Player.KillMe(PlayerDeathReason.ByNPC(npc.whoAmI), 1000, -npc.direction);
            }
