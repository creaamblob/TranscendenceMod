...
namespace TranscendenceMod.NPCs.Boss.FrostSerpent
{
    ...
    public class FrostSerpent_Head : HeadSegment
    {
        ...
        public override void ModifyNPCLoot(NPCLoot npcLoot)
        {
            ...
            normalMode.OnSuccess(ItemDropRule.FewFromOptions(2, 1, ModContent.ItemType<MountaintopGlacier>(), ModContent.ItemType<Snowshot>(), ModContent.ItemType<Snowstorm>()));
        ...
        public override void AI()
        {
            ...
            if (Attack != SerpentAttacks.Slam && Attack != SerpentAttacks.Charges && Attack != SerpentAttacks.MultiRays && Attack != SerpentAttacks.DeathAnim)
            {
                ...
                NPC.velocity = Vector2.Lerp(NPC.velocity, NPC.DirectionTo(player.Center + Vector2.One.RotatedBy(rot * (float)Math.Tan(rot / 20)) * dist) * (90f + (speed2 * 0.66f) * speedMult) * NPC.scale, changeSpeed * speedMult);
    ...
    public class FrostSerpent_Body : BodySegment
    {
        ...
        public override void AI()
        {
            ...
            if (NPC.life < 15)
            {
                ...
                {
                    NPC npc = Main.npc[i];
                    if (npc != null && npc.active && (npc.type == ModContent.NPCType<FrostSerpent_Body>() || npc.type == ModContent.NPCType<FrostSerpent_Head>()
                         || npc.type == ModContent.NPCType<FrostSerpent_Tail>()))
                    {
                        npc.life = 1;
                        npc.realLife = 1;
                        npc.dontTakeDamage = true;
    ...
    public class FrostSerpent_Tail : TailSegment
    {
        ...
        public override void AI()
        {
            ...
            if (NPC.life < 15)
            {
                ...
                {
                    NPC npc = Main.npc[i];
                    if (npc != null && npc.active && (npc.type == ModContent.NPCType<FrostSerpent_Body>() || npc.type == ModContent.NPCType<FrostSerpent_Tail>()))
                    {
                        npc.life = 1;
                        npc.realLife = 1;
                        npc.dontTakeDamage = true;
