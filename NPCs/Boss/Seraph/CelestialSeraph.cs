...
namespace TranscendenceMod.NPCs.Boss.Seraph
{
    ...
    public class CelestialSeraph : ModNPC
    {
        ...
        public override void ModifyNPCLoot(NPCLoot npcLoot)
        {
            ...
            /*Materials*/
            normalMode.OnSuccess(ItemDropRule.Common(ModContent.ItemType<ShimmerChunk>(), 1, 10, 16));
            /*Main Drops*/
        ...
        public void GrowAndShrink()
        {
            ...
            if (Timer_AI > 200 && Timer_AI < (AttackDuration - 30))
            {
                if (++ProjectileCD[2] % 60 == 0)
                {
                    for (int i = 0; i < 6; i++)
                    {
                        Vector2 vel2 = vel.RotatedBy(MathHelper.TwoPi * i / 6f);
                        int p = Projectile.NewProjectile(NPC.GetSource_FromAI(), arenaCenter + Vector2.One.RotatedBy(vel.ToRotation() - MathHelper.PiOver4 + MathHelper.TwoPi * i / 6f) * 220f * skyFade, vel2 * 4f, ModContent.ProjectileType<SunBeam>(), 140, 2f, -1, -90, NPC.whoAmI, 10f);
        ...
        private void KillClutter()
        {
            for (int p = 0; p < Main.maxProjectiles; p++)
            {
                Projectile projectile = Main.projectile[p];
                /*Clear Clutter*/
                if (projectile.ai[1] == NPC.whoAmI && projectile.hostile && (projectile.type != deathanim && projectile.type != sun || ActivatedPhase3Skin || player.dead) || projectile.type == mist || projectile.type == galaxyshard || projectile.type == stellarfireball)
                {
                    projectile.Kill();
                }
            }
