    ...
    public class ProjectNucleus : ModNPC
    {
            ...
            if (++BigHeartCDs[0] > 150)
            {
                ...
                for (int i = 0; i < 32; i++)
                {
                    for (int j = -1; j < 2; j += 2)
                    {
                        ...
                        int p = Projectile.NewProjectile(NPC.GetSource_FromAI(), startPos + pos, vel, heart, 90, 2f, -1, BigHeartCDs[1], NPC.whoAmI, i);
        ...
        public void MissileHearts()
        {
            for (int i = 0; i < 32; i++)
            {
                for (int j = -1; j < 2; j += 2)
                {
                    ...
                    Projectile.NewProjectile(NPC.GetSource_FromAI(), pos2, vel, missile, 75, 2f, -1, 1f, NPC.whoAmI);
                }
            }
        }
        ...
        public void Homing()
        {
            ...
            if (++ProjectileCD % 30 == 0)
                TranscendenceUtils.ProjectileRing(NPC, 4, NPC.GetSource_FromAI(), NPC.Center, missile, 85, 2f, 1f, 0f, NPC.whoAmI, 0f, -1, Main.rand.NextFloat(MathHelper.TwoPi));
        }
        ...
        public void Beams()
        {
            ...
            {
                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, NPC.DirectionTo(player.Center) * 4f, nucleusBeam, 100, 2f, -1, -20, NPC.whoAmI);
                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center + new Vector2(0, 62), NPC.DirectionTo(player.Center) * 2f, missile, 70, 2f, -1, -20, NPC.whoAmI);
            }
        }
        ...
        public void RingBeams()
        {
            ...
            if (Timer_AI == 90)
                TranscendenceUtils.ProjectileRing(NPC, 6, NPC.GetSource_FromAI(), NPC.Center, nucleusBeam, 90, 2f, 1f, -30, NPC.whoAmI, 0, -1, Main.rand.NextFloat(MathHelper.TwoPi));

            if (++ProjectileCD % 45 == 0)
                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center + new Vector2(0, 62), NPC.DirectionTo(player.Center) * 0.75f, missile, 70, 2f, -1, -20, NPC.whoAmI);
        }
        ...
        public void MineField()
        {
            ...
            {
                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, Main.rand.NextVector2Circular(95f, 95f), mine, 85, 1, -1, 0, NPC.whoAmI);
            }
        }
        ...
        public void LaserPillars()
        {
            ...
            if (NPC.Center.Y < (Center.Y - 625) && ProjectileCD2 != 1)
            {
                for (int i = -1000; i < 1000; i += 200)
                {
                    ...
                    Projectile.NewProjectile(NPC.GetSource_FromAI(), pos, new Vector2(0, 5), nucleusBeam, 80, 0f, -1, -60, NPC.whoAmI);
                }
                            ...
                            for (int l = -1000; l < 1400; l += 400)
                            {
                                ...
                                for (int i = 0; i < 16; i++)
                                {
                                    for (int j = -1; j < 2; j += 2)
                                    {
                                        ...
                                        Projectile.NewProjectile(NPC.GetSource_FromAI(), startPos + pos, vel, heart, 90, 2f, -1, 1f, NPC.whoAmI, i);
                                    }
                                }
                            }
        ...
        public void Sawblades()
        {
            ...
            if (Timer_AI > 60 && ++ProjectileCD == 15)
            {
                ...
                {
                    Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, NPC.DirectionTo(player.Center) * 14f, saw, 80, 2f, -1, 0f, NPC.whoAmI, MathHelper.ToRadians(i));
                }
            }
            if (ProjectileCD > 75)
                ProjectileCD = 0;
