...
namespace TranscendenceMod.NPCs.SpaceBiome
{
    public class Blackhole : SpaceBiomeNPC
    {
            ...
            for (int i = 0; i < Main.maxItems; i++)
            {
                ...
                if (i < Main.maxPlayers)
                {
                    ...
                    if (player != null && player.active)
                    {
                        ...
                        if (player.Distance(NPC.Center) < (75 * NPC.scale))
                        {
                            player.velocity += player.DirectionTo(NPC.Center) * 1.5f;
        ...
        {
            BlackholeDrawer.DrawBlackhole(NPC, 1f * NPC.scale, spriteBatch);
            return false;
        }
    }
}
